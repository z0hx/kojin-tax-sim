...
- 税制パラメータは 2025年分・2026年分のみ収録しています。年度データを作成できるのも収録済みの年分だけです（過去年分も作成できますが、2024年分以前は収録していないため入力できません）。2024年分は令和6年分の定額減税を計算モデルに持っていないため、パラメータだけを追加しても源泉徴収票と一致しません
...
1. `public/taxParams/{年}.json` を追加する。形は `src/taxParams/schema.ts` の `TaxParams` に従う（前年のファイルをコピーして差分を当てるのが確実です）。
   - あわせて `src/taxParams/supportedYears.ts` の `SUPPORTED_TAX_YEARS` にその年を追加する。アプリで年度データを作成できる年分はこの一覧で決まります（更新漏れは手順3のテストが検出します）。
2. `meta` を必ず更新する。
...
現在 61ファイル・586テスト、行カバレッジ 94.2%（NFR-06 の「80%以上」を満たしています）。
