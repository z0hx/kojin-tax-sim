...
| `calcIncomeLevy` | `(before: Yen, adjustment: Yen) => Yen` | `max(0, before - adjustment)`。★特例分20%枠の基準（`standard`モード時） |
...
| `setSafetyRatio` | `(ratio: number) => void` | FR-13 |
...
| S-05 計算明細 | `CalculationTraceTable`, `IncomeTaxBreakdownSection`, `ResidentTaxBreakdownSection`, `CapModeComparisonToggle` | `calculationResult.trace`, `calculationResultConservative` | `setCapMode` |
...
所得割額（20%枠基準） = 235,788 − 2,500 = 233,288
...
特例分の上限 = 233,288 × 20% = 46,657 → 特例分(確定) = 46,657 …★specialCapReached = true（T-11）
住宅ローン控除適用後の所得割額 = 233,288 − 19,800 − 46,657 = 166,831
住民税から住宅ローン控除 = min(147,400, 97,500, 166,831) = 97,500 → 切り捨て損失 = 147,400 − 97,500 = 49,900円
```
...
```
所得税差 = (0 − 0) = 0
住民税差 = (141,600 − (69,300+5,900)) = 141,600 − 75,200 = 66,400
  ※住民税所得割(寄附あり)確定 = floor100(166,831 − 97,500) = floor100(69,331) = 69,300
自己負担(200,000) = 200,000 − (0 + 66,400) = 133,600円
```
