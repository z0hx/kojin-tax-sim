...
type FurusatoCapMode = 'standard' | 'conservative';   // engineの引数専用。永続化する設定としては持たない
```
...
| 関数 | シグネチャ | 処理内容 |
|---|---|---|
| `applyLeavePeriods` | `(income: IncomeInput, year: number) => IncomeInput` | `leavePeriods`の各期間を月に展開し、対象月の`grossSalary`/`socialInsurance`を0、`isSocialInsuranceExempt`をtrueにした**新しい**`IncomeInput`を返す（疑似コードは破壊的更新だが、純粋関数化のため`monthly`配列を都度複製する）。賞与についても`isBonusExempt`の結果を反映した新配列を返す。対象月は社会保険料の内訳（Issue #48）も落とし、入力方法にかかわらず0円になることを保証する（この結果は生データへ書き戻さないため、育休期間を編集・削除すれば内訳は元のまま復元される） |
...
| `resolveMonthlySocialInsurance` | `(rec: MonthlyRecord) => number` | その月の社会保険料。`socialInsuranceInputMode === 'breakdown'`かつ内訳がある場合は内訳の合計、それ以外（既定・内訳欠落）は一括入力額`socialInsurance`を返す（Issue #48）。社会保険料を参照する処理はすべてこの関数を通し、一括入力額と内訳のどちらが使われるかの判断を一箇所に閉じる |
| `sumMonthlySocialInsurance` | `(income: IncomeInput) => Yen` | `monthly[]`の`resolveMonthlySocialInsurance`の合計 |
...
| `estimateAnnualIncome` | `(income: IncomeInput, params: EstimationParams) => IncomeEstimate` | FR-03。`status: 'actual'`の月数から`confidenceRatio`（実績月数/12）を算出し、`status: 'estimated'`の月を直近実績月の値で埋めた`IncomeInput`と`confidenceRatio`を返す。社会保険料は`fillMode: 'lastActual'`なら直近実績月の入力方法（一括／内訳）ごと写し、`'average'`は月をまたいだ平均値で内訳に分解できないため一括入力として入れる（Issue #48） |
...
| `checkW04` | `profile.deductions.medical.paid > 0 && profile.furusato.method === 'oneStop'`（申告方式は入力後に医療費控除を追加した場合など後から不整合になりうるため、選択肢を塞ぐのではなく警告で気付かせる。02仕様書§6） |
...
`buildCalculationResult`は呼び出しコストが軽いため、`update*`アクションのたびに両モードを計算して`calculationResult`と`calculationResultConservative`の両方をstateに保持する。S-05計算明細画面は両者と差額を常に並記するため、「どちらを既定表示にするか」の設定値は持たない。
...
| S-05 計算明細 | `CalculationTraceTable`, `IncomeTaxBreakdownSection`, `ResidentTaxBreakdownSection`, `TaxParamsProvenance`（R-10: 最終確認日・出典の常時表示） | `calculationResult.trace`, `calculationResultConservative`, `taxParams[year].meta` | なし（表示のみ） |
...
| S-08 設定 | `MunicipalityForm`, `SafetyRatioSlider` | `appData.persons[...].defaults`, `...years[activeYear].municipality` | `updateMunicipality`, `updatePersonDefaults` |
...
sequenceDiagram
    ...
    alt データなし（初回起動）
        Store->>Store: 空のAppDataを生成し、persons.length === 0 からオンボーディング要と判定
...
| FR-19 税制パラメータの外部化 | §2.1 `taxParams/`、§8 `TaxParamsError`。アプリ内での編集手段は設けない（要件定義書§6.4）ため、`TaxParamsEditor` は実装しない |
