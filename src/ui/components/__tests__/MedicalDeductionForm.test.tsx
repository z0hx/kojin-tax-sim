...
describe('MedicalDeductionForm', () => {
  it('総所得金額等が200万円未満の場合、足切り額は総所得×5%が採用されたと表示する(FR-06)', () => {
    render(
      <MedicalDeductionForm value={baseValue()} totalIncome={1_000_000 as Yen} params={PARAMS} oneStopWarningActive={false} onChange={vi.fn()} />
    );
  ...
  it('総所得金額等が200万円以上の場合、定額10万円が採用されたと表示する', () => {
    render(
      <MedicalDeductionForm value={baseValue()} totalIncome={5_000_000 as Yen} params={PARAMS} oneStopWarningActive={false} onChange={vi.fn()} />
    );
  ...
  it('セルフメディケーション税制の方が有利な場合、自動判定でchosenMode=セルフメディケーションと表示する', () => {
    ...
    render(<MedicalDeductionForm value={value} totalIncome={5_000_000 as Yen} params={PARAMS} oneStopWarningActive={false} onChange={vi.fn()} />);
  ...
  it('補填金額が支払医療費を超える編集はブロックされ、onChangeが呼ばれない', () => {
    ...
    render(<MedicalDeductionForm value={value} totalIncome={5_000_000 as Yen} params={PARAMS} oneStopWarningActive={false} onChange={onChange} />);
  ...
  it('oneStopWarningActive(calculationResult.warningsのW-04)がtrueの場合、ワンストップ特例が使えない旨の警告を表示する', () => {
    const value: MedicalInput = { paid: 50_000, reimbursed: 0, selfMedication: 0, mode: 'auto' };
    render(<MedicalDeductionForm value={value} totalIncome={5_000_000 as Yen} params={PARAMS} oneStopWarningActive onChange={vi.fn()} />);
    expect(screen.getByText(/ワンストップ特例は利用できません/)).toBeInTheDocument();
  });

  it('oneStopWarningActiveがfalseの場合は警告を表示しない(条件判定はdomain/warnings.ts側の責務)', () => {
    const value: MedicalInput = { paid: 50_000, reimbursed: 0, selfMedication: 0, mode: 'auto' };
    render(<MedicalDeductionForm value={value} totalIncome={5_000_000 as Yen} params={PARAMS} oneStopWarningActive={false} onChange={vi.fn()} />);
